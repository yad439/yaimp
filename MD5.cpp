...
void MD5::encode(uint8_t *output,const uint32_t *input, size_type len) {
	for (size_type i = 0, j = 0; j < len; i++, j += 4) {
		output[j] = gsl::narrow_cast<uint8_t>(input[i] & 0xffu);
		output[j + 1] = gsl::narrow_cast<uint8_t>((input[i] >> 8u) & 0xffu);
		output[j + 2] = gsl::narrow_cast<uint8_t>((input[i] >> 16u) & 0xffu);
		output[j + 3] = gsl::narrow_cast<uint8_t>((input[i] >> 24u) & 0xffu);
	}
}
...
void MD5::update(const unsigned char input[], size_type length) {
	...
	if ((_count[0] += (length << 3u)) < (length << 3u)) _count[1]++;
	_count[1] += (length >> 29u);
...
inline void MD5::update(const char input[], size_type length) {
	update(pointer_cast<const unsigned char *>(input), length);
}
...
std::string MD5::hexdigest() const {
	...
	for (size_t i = 0; i < 16; ++i){
		buf[2 * i]     = hexmap[(_digest[i] & 0xF0u) >> 4u];
		buf[2 * i + 1] = hexmap[_digest[i] & 0x0Fu];
	}
	return {buf,32};
}
